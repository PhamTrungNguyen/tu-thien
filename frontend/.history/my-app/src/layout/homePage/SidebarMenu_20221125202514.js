...
const SidebarMenu = ({ styled, props }) => {
    console.log("🚀 ~ file: SidebarMenu.js ~ line 8 ~ SidebarMenu ~ props", props.account.displayName)
    const navigate = useNavigate();
    const user = JSON.parse(localStorage.getItem("userLogin"));
    const account = user.account
    console.log("🚀 ~ file: SidebarMenu.js ~ line 12 ~ SidebarMenu ~ account", account)
    // console.log("🚀 ~ file: SidebarMenu.js ~ line 11 ~ SidebarMenu ~ account", account)
    // console.log("🚀 ~ file: SidebarMenu.js ~ line 10 ~ SidebarMenu ~ user", user)
    ...
    return (
        <nav>
            ...
            <div className="sidebar-footer">
                    ...
                    <div className="admin-profile hide">
                        ...
                        <div className="admin-info">
                            <h3 className="cursor-pointer">
                                <a href="/signpage">{` ${props?.account?.username
                                    } `}</a>
                            </h3>
                            {props?.account?.roleId === 1 && <h5>Người dùng</h5>}
                            {props?.account?.roleId === 2 && <h5>Quản lý</h5>}
                            {props?.account?.roleId === 3 && <h5>Admin</h5>}
                        </div>
                    </div>
                    <button
                        className="log-out"
                ...
                {user &&
                    <div
                        className="ml-[40px] mt-[20px] mb-[-20px] cursor-pointer"
...
{` ${props.displayName === "undefined" ? props.displayName : props.username || "Vui lòng đăng nhập"
                    ...
                    <span className="show">
                        {` ${props?.account?.displayName
                            ? props?.account?.displayName
                            : props?.account?.username || "Vui lòng đăng nhập"
                            }  `}{" "}
                    </span>

                    <span>{`${props?.account?.username ? "Đăng xuất" : "Đăng nhập"}`}</span>
                </div>
            </div>
        </nav>
    );
